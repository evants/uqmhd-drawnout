...
extern COUNT NUM_DRIVE_SLOTS=11;
COUNT DRIVE_TOP_X;
COUNT DRIVE_SIDE_X;
extern COUNT badguysImprove=0;

extern COUNT NUM_MODULE_SLOTS=16;
BOOLEAN thrusterUpgraded=0;
BOOLEAN modulesUpgraded=0;
extern bool blasterUpgraded=0;
extern bool energyUpgraded=0;
extern bool earthlingUpgraded=0;
//extern COUNT EMPTY_SLOT=NUM_MODULES;
void upgradeBlaster(void)
{
blasterUpgraded=1;
}

void upgradethruster(void)
{
	thrusterUpgraded=1;
	/*
	NUM_DRIVE_SLOTS=13;
	...
	for (i=0;i<=NUM_DRIVE_SLOTS;++i)
		printf("slot:%d val:%d\n",i,GLOBAL_SIS (DriveSlots[i]));

        for (i = 0; i < NUM_DRIVE_SLOTS; ++i)
                if (GLOBAL_SIS (DriveSlots[i]) != FUSION_THRUSTER);
			printf("drive slots changed: %d\n",i);
			GLOBAL_SIS (DriveSlots[i]) = EMPTY_SLOT+0;
	GLOBAL_SIS (DriveSlots[11]) = EMPTY_SLOT+0;
	GLOBAL_SIS (DriveSlots[12]) = EMPTY_SLOT+0;
	...
	for (i=0;i<=NUM_DRIVE_SLOTS;++i)
        ...
        for (i = 0; i < NUM_JET_SLOTS; ++i)
			...
			printf("jet slots changed: %d\n",i);
			GLOBAL_SIS (DriveSlots[i]) = EMPTY_SLOT+1;
	*/
}

void upgrademodules(void)
{
	/*printf("before\n");
	int i;
	for (i=0;i<=NUM_MODULE_SLOTS+1;i++)
		printf("slot:%d val:%d\n",i,GLOBAL_SIS (ModuleSlots[i]));*/
	//int old_num_module_slots=NUM_MODULE_SLOTS;
	NUM_MODULE_SLOTS=20;
	//printf("this should not be 20: %d",old_num_module_slots);
	//EMPTY_SLOT=NUM_MODULES;
	if ( modulesUpgraded == 0)
	{
	BYTE tmp=GLOBAL_SIS (ModuleSlots[16]);//stop value?
	GLOBAL_SIS (ModuleSlots[20])=tmp;
	GLOBAL_SIS (ModuleSlots[16])=EMPTY_SLOT+2;

	tmp=GLOBAL_SIS (ModuleSlots[15]);//front
	GLOBAL_SIS (ModuleSlots[19])=tmp;
	GLOBAL_SIS (ModuleSlots[15])=EMPTY_SLOT+2;
	tmp=GLOBAL_SIS (ModuleSlots[14]);//spread
	GLOBAL_SIS (ModuleSlots[18])=tmp;
	GLOBAL_SIS (ModuleSlots[14])=EMPTY_SLOT+2;
	tmp=GLOBAL_SIS (ModuleSlots[13]);//side
	GLOBAL_SIS (ModuleSlots[17])=tmp;
	GLOBAL_SIS (ModuleSlots[13])=EMPTY_SLOT+2;
	tmp=GLOBAL_SIS (ModuleSlots[12]);//additional
	GLOBAL_SIS (ModuleSlots[16])=tmp;
	GLOBAL_SIS (ModuleSlots[12])=EMPTY_SLOT+2;
	}
	modulesUpgraded=1;
	earthlingUpgraded=1;
...
#define TURN_WAIT 17
	...
	*/
	...
	snprintf (buf, sizeof buf, "%-7.7s",
			describeWeapon (GLOBAL_SIS (ModuleSlots[NUM_MODULE_SLOTS-1]))); //This is where it pulls the text field of the gun
	font_DrawText (&t);
	t.baseline.y += leading;
	snprintf (buf, sizeof buf,
			"%-7.7s", describeWeapon (GLOBAL_SIS (ModuleSlots[NUM_MODULE_SLOTS-2])));
	font_DrawText (&t);
	t.baseline.y += leading;
	snprintf (buf, sizeof buf,
			"%-7.7s", describeWeapon (GLOBAL_SIS (ModuleSlots[NUM_MODULE_SLOTS-3])));
	font_DrawText (&t);
	t.baseline.y += leading;
	snprintf (buf, sizeof buf,
...
DrawModules (void)
{
	...
	s.origin.y = 1;
	//This adjusts the SIS offset to add additional modules
	if (modulesUpgraded == 1)
		s.origin.y = RES_STAT_SCALE(8);
	for (i = 0; i < NUM_MODULE_SLOTS; ++i)
	{
...
GetCPodCapacity (POINT *ppt)
{
	...
	ppt->x = (27 << RESOLUTION_FACTOR) + (slotNr * ship_piece_offset_scaled) -
				...
				((colNr * 2) << RESOLUTION_FACTOR) + RES_CASE(0,19,53) -(modulesUpgraded*100); // JMS_GFX // shift if modules upgraded
...
GetFTankCapacity (POINT *ppt)
{
	...
	if (volume == FUEL_TANK_CAPACITY)
	{
		...
		ppt->x += RES_CASE(0,20,54) -(100*modulesUpgraded); // JMS_GFX
		ppt->y = (27 << RESOLUTION_FACTOR) - rowNr + RES_CASE(0,11,27); // JMS_GFX
	}
	else
	{
		//ppt->x += RES_CASE(0,20,53) ; // JMS_GFX
		ppt->x += RES_CASE(0,20,53) -(100*modulesUpgraded); // JMS_GFX
