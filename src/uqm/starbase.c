		...
		BOOLEAN DrawBluePrint)
{
	...
	switch (which_piece)
	{
		...
		default:
			...
			//COUNT MODULE_SIDE_X = ((17 << RESOLUTION_FACTOR) + RES_CASE(0,21,55)); //normal ship
			COUNT MODULE_SIDE_X = ((17 << RESOLUTION_FACTOR) + RES_CASE(0,21,55) -(100*modulesUpgraded)); //extra slots
			COUNT MODULE_TOP_X  = ((17 << RESOLUTION_FACTOR) + RES_CASE(0,21,55) -(100*modulesUpgraded));
			/*if (thrusterUpgraded == 0)//not upgraded
				DRIVE_TOP_X=((33 << RESOLUTION_FACTOR) + RES_CASE(0,42,100));
			else //upgraded
				//DRIVE_TOP_X=((22 << RESOLUTION_FACTOR) + RES_CASE(0,42,100));*/
			//DRIVE_TOP_X=((33 << RESOLUTION_FACTOR) + RES_CASE(0,42,100)-45*thrusterUpgraded);
			//if (thrusterUpgraded == 0) // not upgraded //will adjust offset
			DRIVE_SIDE_X=((31 << RESOLUTION_FACTOR)); //b4 upgrade //JMS_GFX
			//else
			//	DRIVE_SIDE_X=((22 << RESOLUTION_FACTOR))-ship_piece_offset_scaled;  // after upgrade //JMS_GFX

			DRIVE_TOP_X=((33 << RESOLUTION_FACTOR) + RES_CASE(0,42,100));//-ship_piece_offset_scaled*thrusterUpgraded);
