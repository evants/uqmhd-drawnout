...
InitWeaponSlots (RACE_DESC *RaceDescPtr, const BYTE *ModuleSlots)
{
	...
	for (i = 0; i < NUM_MODULE_SLOTS; ++i)
	{
		...
		lpMB->life = BLASTER_LIFE + ((BLASTER_LIFE >> 2) * (which_gun - 1))*(1+(blasterUpgraded*3));
	...
	RaceDescPtr->characteristics.weapon_energy_cost*=1+blasterUpgraded;
}
...
InitModuleSlots (RACE_DESC *RaceDescPtr, const BYTE *ModuleSlots)
{
	...
	if (energyUpgraded == 1)
        RaceDescPtr->ship_info.max_energy=80;
}
