		...
		else
		{
			...
			switch (stack)
			{
				...
				case 12:
					...
					break;
                case 13:
					pStr = OK_BUY_NEW_TECH_14;
					needed_credit = 75 * BIO_CREDIT_VALUE;
					break;
                case 14:
					pStr = OK_BUY_NEW_TECH_15;
					needed_credit = 150 * BIO_CREDIT_VALUE;
					break;
                case 15:
					pStr = OK_BUY_NEW_TECH_16;
					needed_credit = 150 * BIO_CREDIT_VALUE;
					break;
                case 16:
					pStr = OK_BUY_NEW_TECH_17;
					needed_credit = 150 * BIO_CREDIT_VALUE;
					break;
                case 17:
					pStr = OK_BUY_NEW_TECH_18;
					needed_credit = 150 * BIO_CREDIT_VALUE;
					break;
				default:
		...
		switch (stack)
		{
			...
			case 13:
				GLOBAL (ModuleCost[SHIVA_FURNACE]) =
						4000 / MODULE_COST_SCALE;
				if (thrusterUpgraded == 0)
				{
                    NPCPhrase(NEW_TECH_COST_MORE);
					NPCPhrase (NEW_TECH_14);
					break;
				}
				++stack;
            case 14:
                upgradethruster();
                if (earthlingUpgraded == 0)
                {
                    NPCPhrase(NEW_TECH_15);
                    break;
                }
                ++stack;
            case 15:
                earthlingUpgraded=1;
                if (blasterUpgraded == 0)
                {
                    NPCPhrase(NEW_TECH_16);
                    break;
                }
                ++stack;
            case 16:
            ...
            case 17:
                ...
                {
                    NPCPhrase(NEW_TECH_18);
                    break;
                }
                ++stack;
			case 18:
				upgrademodules();
				NPCPhrase (NEW_TECH_ALL_GONE); //last purchase
				SET_GAME_STATE (MELNORME_TECH_STACK, stack); //last purchase
				goto BuyBuyBuy; //last purchase
		}
