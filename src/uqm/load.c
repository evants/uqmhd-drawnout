...
int checkUpgrades(SIS_STATE *SSPtr)
{
    /*int i;
    for (i=15;i<21;++i)
        printf("debug [%d] = %d\n",i,SSPtr->ModuleSlots[i]);*/
    if (SSPtr->ModuleSlots[16] == 0)
        {printf("debug: no extra modules detected\n");
        NUM_MODULE_SLOTS=16;
        modulesUpgraded=0;
        }
    else if (SSPtr->ModuleSlots[20] == 0)
        {printf("debug: 4 extra modules detected\n");
        NUM_MODULE_SLOTS=20;
        modulesUpgraded=1;
        }
...
LoadSisState (SIS_STATE *SSPtr, void *fp)
{
	if (
			...
			read_a8  (fp, SSPtr->ModuleSlots, MAX_NUM_MODULE_SLOTS) != 1 ||
			//(read_a8  (fp, SSPtr->ModuleSlots, NUM_MODULE_SLOTS)+1) >=1000 ||
			read_a8  (fp, SSPtr->DriveSlots, MAX_NUM_DRIVE_SLOTS) != 1 ||
			checkUpgrades(SSPtr) != 1 ||
