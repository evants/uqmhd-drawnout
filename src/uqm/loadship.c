...
load_ship (SPECIES_ID SpeciesID, BOOLEAN LoadBattleData)
{
	...
	if (badguysImprove>0)
      {
      printf("debug: comparing %d to %d \n",SpeciesID,ILWRATH_ID);
      //if (SpeciesID == ILWRATH_ID)
      switch (SpeciesID)
        {
        case ILWRATH_ID:
            RDPtr->ship_info.energy_level+=2;
            RDPtr->ship_info.max_energy+= 2;
            //RDPtr->ship_info. MissileBlock.life=99;
            break;
        case KOHR_AH_ID:
            RDPtr->ship_info.energy_level+=2;
            RDPtr->ship_info.max_energy+= 2;
        case MYCON_ID:
            RDPtr->ship_info.crew_level+=2*badguysImprove;
            RDPtr->ship_info.max_crew+=2*badguysImprove;
        default:
            break;
        }
      }
